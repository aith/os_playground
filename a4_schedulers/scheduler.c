...
typedef struct __stats_by_thread_t {
  ...
  unsigned int io_waiting_time_start;
  unsigned int io_waiting_time_end;
  struct __stats_t
...
stats_by_thread_t *init_stats_by_thread_t(thread_t *t) {
  ...
  stats_by_t->io_waiting_time_start = 0;
  stats_by_t->io_waiting_time_end = 0;
  t_count++;
  return stats_by_t;
}
...
void sys_exit(thread_t *t) {
  ...
  printf("thread %d spent %d waiting\n", fin->stats->tid,
         fin->waiting_time_end - fin->waiting_time_start);
  printf("thread %d spent %d io_waiting\n", fin->stats->tid,
         fin->io_waiting_time_end - fin->io_waiting_time_start);
  unsigned int waiting_time = fin->waiting_time_end - fin->waiting_time_start +
                              fin->io_waiting_time_end -
                              fin->io_waiting_time_start;
...
void put_on_io_wait(thread_t *t) {
  stats_by_thread_t *paused = (stats_by_thread_t *)queue_dequeue(running_q);
  queue_enqueue(io_q, paused);
  paused->io_waiting_time_start += sim_time() + 1;
}
 ...
 */
void sys_read(thread_t *t) { put_on_io_wait(t); }
 ...
 */
void sys_write(thread_t *t) { put_on_io_wait(t); }
...
void io_complete(thread_t *t) {
  stats_by_thread_t *unpaused = (stats_by_thread_t *)queue_dequeue(io_q);
  queue_enqueue(ready_q, unpaused);
  unpaused->waiting_time_start += sim_time();
}
...
void io_starting(thread_t *t) {
  stats_by_thread_t *head = (stats_by_thread_t *)queue_head(io_q);
  head->io_waiting_time_end += sim_time() - 1;
}
...
static int tid_lt(void *a, void *b) {
  return ((stats_by_thread_t *)a)->stats->tid <
         ((stats_by_thread_t *)b)->stats->tid;
}
